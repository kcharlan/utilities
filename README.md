...
- Prefer `pip install --no-cache-dir --disable-pip-version-check` inside self-bootstrapping launchers so startup output is not polluted by stale shared pip HTTP cache entries or pip housekeeping notices. Apply this pattern to private dependency installs in self-bootstrapping tools such as `routerview` and `router-log-analyzer`.
