...
class TestGetOpencodeModelConfig:
    def test_returns_correct_format(self, adapter):
        ...
        assert "can_reason" in first
        assert "supports_attachments" in first
        assert "context_window" in first
        assert "default_max_tokens" in first
        assert "cost_per_1m_in" in first
        assert "cost_per_1m_out" in first
        assert "cost_per_1m_in_cached" in first
        assert "cost_per_1m_out_cached" in first
