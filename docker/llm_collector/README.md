...
*   `MY_API_KEY.txt`: Stores your API key. Read by `reset_collector.sh` to authenticate reset requests.
...
*   `com.kevinharlan.llmcollector.reset.plist`: macOS `launchd` plist that runs `reset_collector.sh` at midnight.
...
2.  **Start Collector**: Navigate to `llm_collector_container` and run `./up.sh` (or `docker compose up --build -d`).
