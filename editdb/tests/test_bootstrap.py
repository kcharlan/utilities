...
def load_module(monkeypatch, runtime_home: Path):
    monkeypatch.setenv("EDITDB_HOME", str(runtime_home))
    module_name = f"editdb_bootstrap_{uuid.uuid4().hex}"
...
def test_help_does_not_create_runtime_state(tmp_path):
    ...
    env = os.environ.copy()
    env["EDITDB_HOME"] = str(runtime_home)
...
def test_bootstrap_creates_runtime_state_and_reexecs(monkeypatch, tmp_path):
    ...
    with pytest.raises(Reexec) as exc_info:
    ...
    state = module.json.loads(paths.bootstrap_state.read_text(encoding="utf-8"))
    assert state == module.desired_bootstrap_state()
    executable, argv = exc_info.value.args[0]
    assert executable == str(paths.venv_python)
    assert argv[0] == str(paths.venv_python)
    assert argv[1] == str(SCRIPT_PATH.resolve())
