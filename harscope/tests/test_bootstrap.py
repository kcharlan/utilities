...
SCRIPT_PATH = Path(__file__).resolve().parents[1] / "harscope"


def load_module(monkeypatch, runtime_home: Path):
    monkeypatch.setenv("HARSCOPE_HOME", str(runtime_home))
    module_name = f"harscope_bootstrap_{uuid.uuid4().hex}"
...
def test_help_does_not_create_runtime_state(tmp_path):
    ...
    env = os.environ.copy()
    env["HARSCOPE_HOME"] = str(runtime_home)
